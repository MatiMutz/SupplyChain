...
namespace SupplyChain.Pages.Clases
{
    public class ClasePageBase : ComponentBase
    {
        ...
        {
            clases = await Http.GetFromJsonAsync<List<Clase>>("api/Clases");
        ...
        public async Task ActionBegin(ActionEventArgs<Clase> args)
        {
            if (args.RequestType == Syncfusion.Blazor.Grids.Action.Save)
            {
                ...
                if (!found)
                {
                    response = await Http.PostAsJsonAsync("api/Clases", args.Data);

                }
                else
                {

                    response = await Http.PutAsJsonAsync($"api/Clases/{args.Data.CG_CLASE}", args.Data);
                }
        ...
        private async Task EliminarOperario(ActionEventArgs<Clase> args)
        {
            try
            {
                if (args.Data != null)
                {
                    ...
                    if (isConfirmed)
                    {
                        ...
                        await Http.DeleteAsync($"api/Clases/{args.Data.CG_CLASE}");
                    }
                }
            }
            catch (Exception ex)
