...
namespace SupplyChain.Pages.TipoCelda
{
    public class TipoCeldaPageBase : ComponentBase
    {
        [Inject] protected CustomHttpClient Http { get; set; }
        [Inject] protected IJSRuntime JsRuntime { get; set; }
        protected SfGrid<TipCelda> Grid;

        public bool Enabled = true;
        public bool Disabled = false;


        protected List<TipCelda> tipoceldas = new List<TipCelda>();
        ...
        {
            tipoceldas = await Http.GetFromJsonAsync<List<TipCelda>>("api/TipoCelda");
       

            await base.OnInitializedAsync();
        }

        public void ActionBeginHandler(ActionEventArgs<TipCelda> args)
        {
        ...
        public async Task ActionBegin(ActionEventArgs<TipCelda> args)
        {
            if (args.RequestType == Syncfusion.Blazor.Grids.Action.Save)
            {
                HttpResponseMessage response;
                bool found = tipoceldas.Any(p => p.Cg_tipocelda == args.Data.Cg_tipocelda);
                TipCelda ur = new TipCelda();
        ...
        private async Task EliminarOperario(ActionEventArgs<TipCelda> args)
        {
            try
            {
                if (args.Data != null)
                ...
                if (this.Grid.SelectedRecords.Count > 0)
                {
                    foreach (TipCelda selectedRecord in this.Grid.SelectedRecords)
                    {
                        ...
                        if (isConfirmed)
                        {
                            ...
                            {
                                Grid.Refresh();
                                var tipocelda = await response.Content.ReadFromJsonAsync<TipCelda>();
